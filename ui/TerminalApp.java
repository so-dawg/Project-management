...
public class TerminalApp {
    ...
    private void createTask() {
        try {
            ...
            String deadlineStr;
            do {
                deadlineStr = readLine();
                if (deadlineStr == null) {
                    System.out.println("please input again");
                } else if (deadlineStr.matches("^\\d{4}-\\d{2}-\\d{2}$")
                        || deadlineStr.matches("^\\d{2}-\\d{2}-\\d{4}$")) {
                    break;
                } else {
                    System.out.println(
                            "Invalid date format. Please enter the date in YYYY-MM-DD format or leave it empty.");
                }
            } while (true);
            ...
            selectedProject.addTask(currentUser, title, priority,
                    deadline != null ? deadline.format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd"))
                            : null,
                    description);
                ...
                if (assignChoice != null) {
                    try {
                        ...
                        if (choice == 0) {
                            ...
                            System.out
                                    .println("Assigned To: " + assignee.getFirstName() + " " + assignee.getLastName());
                        } else {
